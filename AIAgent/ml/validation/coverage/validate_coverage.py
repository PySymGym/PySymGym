...
from common.config.validation_config import SVMValidation, SVMValidationViaServer
...
from ml.validation.coverage.game_managers.server.server_game_manager import (
    ServerGameManager,
)
...
class ValidationCoverage:
    ...
    Attributes:
        _game_manager (`BaseGameManager`): The game manager instance used to run game. This is dynamically
                                          selected based on the validation configuration.
    ...
    def _evaluate_game_map(
        ...
        if (
            ...
            states = self._game_manager.get_game_states(game_map2svm.GameMap)
            self.dataset.update_map(game_map2svm.GameMap.MapName, map_result, states)
            del states
        return result
    ...
    def validate_coverage(
        ...
        validation_svm : ValidationSVM
            Validation info.
        progress_bar_colour : str
            Your favorite colour for progress bar.
        """
        self._game_manager = self._get_game_manager(validation_svm)
        with mp.Pool(validation_svm.process_count) as p:
    ...
    def _get_game_manager(self, validation_config: SVMValidation) -> BaseGameManager:
        if isinstance(validation_config, SVMValidationViaServer):
            return ServerGameManager(TrainingModelWrapper(self.model))
