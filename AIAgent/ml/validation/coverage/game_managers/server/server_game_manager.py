...
from ml.validation.coverage.game_managers.server.game_states_utils import (
    get_states,
    update_game_state,
)
...
class ServerGameManager(BaseGameManager):
    _game_states: dict[str, list[HeteroData]] = {}
    ...
    def get_game_states(self, game_map: GameMap) -> list[HeteroData]:
        steps = self._game_states.pop(str(game_map))
        return steps
